...
\caption{Screenshot der Simulationsumgebung mit dem AUV, einem Testobjekt und dem generierten Meeresboden}
\label{screenSim}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Anordnung der 2D-Pixelkoordinaten. Ursprung liegt in der linken oberen Bildecke. Die X-Achse bildet die Bildspalten und die Y-Asche die Bildzeilen ab.}
	\label{imageKoords}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Das Kamerakoordinatensystem. Ursprung des Systems liegt im Mittelpunkt der Kameralinse. Die X-Achse bildet die Bildspalten und die Y-Asche die Bildzeilen im Raum ab. Die Z-Achse zeigt in Blickrichtung der Kamera.}
	\label{CamKoords}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Das Body Koordinatensystem mit Massenschwerpunkt (\textit{cog}) des AUVs. Die X-Achse zeigt frontal voraus, die Y-Achse Richtung Steuerbord und die Z-Achse zeigt nach unten. \textit{p} \textit{r} und \textit{q} beschreiben die Neigungswinkel und Rotationsrichtung an den jeweiligen Achsen (vgl. Abb. \ref{Abb. 2})}
	\label{Abb. 1}
\end{figure}
